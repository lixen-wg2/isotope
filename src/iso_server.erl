...
handle_info(refresh_tick, State = #iso_state{callback = Cb, user_state = US, tree = OldTree}) ->
    ...
    RawTree = Cb:view(NewUS),
    %% Merge scroll/selection state from old tree to preserve positions
    NewTree = iso_tree:merge_state(OldTree, RawTree),
    NewState = State#iso_state{user_state = NewUS, tree = NewTree},
    FinalState = render_diff(NewState),
...
handle_activate(State = #iso_state{focused_container = Container, focused_child = FocusedChild,
    ...
    case iso_engine:activation_target(Tree, Container, FocusedChild) of
        ...
        #tree{id = Id} = TreeEl ->
            NewTreeEl = iso_tree_nav:toggle_selected(TreeEl, Bounds),
            NewTree = iso_tree:update(Tree, Id, NewTreeEl),
...
apply_view_update(NewUS, State = #iso_state{callback = Cb,
                                            focused_container = Container,
                                            focused_child = Child}, MergeFromTree) ->
    RawTree = Cb:view(NewUS),
    NewTree = case MergeFromTree of
        undefined -> RawTree;
...
do_pop(State = #iso_state{nav_stack = [Entry | Rest]}) ->
    ...
    NewTree = iso_tree:merge_state(OldTree, Cb:view(US)),
