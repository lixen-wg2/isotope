...
diff_screens(#screen{width = W, height = H, cells = OldCells},
    ...
    diff_cells(OldCells, NewCells, W, H, 0, 0, #{}, []).

diff_cells(_OldCells, _NewCells, _W, H, _Col, Row, _LastStyle, Acc) when Row >= H ->
    lists:reverse(Acc);
diff_cells(OldCells, NewCells, W, H, Col, Row, LastStyle, Acc) when Col >= W ->
    diff_cells(OldCells, NewCells, W, H, 0, Row + 1, LastStyle, Acc);
diff_cells(OldCells, NewCells, W, H, Col, Row, LastStyle, Acc) ->
    Idx = Row * W + Col,
    ...
    case OldCell =:= NewCell of
        true ->
            %% No change, skip
            diff_cells(OldCells, NewCells, W, H, Col + 1, Row, LastStyle, Acc);
        false ->
            ...
            StyleChange = style_change(LastStyle, Style),
            CharBin = char_to_binary(Char),
            Output = [MoveTo, StyleChange, CharBin],
            diff_cells(OldCells, NewCells, W, H, Col + 1, Row, Style, [Output | Acc])
    end.
