...
-spec height(#list{}, #bounds{}) -> pos_integer().
...
render_items([Item | Rest], X, Y, Width, Selected, Offset, ItemStyle, SelectedStyle, Acc) ->
    ...
    %% Render the line
    Line = render_item_line(X, Y + LineIdx, Width, Label, Style, IsSelected),
...
render_item_line(X, Y, Width, Label, Style, IsSelected) ->
    %% Prefix for selected item
    Prefix = if IsSelected -> <<"▸ ">>; true -> <<"  ">> end,
    PrefixLen = 2,
    
    %% Truncate label to fit
    MaxLabelLen = Width - PrefixLen,
