...
tree_hit_node_test() ->
    ...
    Bounds = default_bounds(),
    ?assertEqual({tree_node, my_tree, root}, iso_hit:find_at(Tree, 3, 1, Bounds)),
...
box_hit_child_test() ->
    ...
    Bounds = default_bounds(),
    %% Should find the child button, not the box
    ?assertEqual({button, inner_btn}, iso_hit:find_at(Tree, 5, 4, Bounds)).
