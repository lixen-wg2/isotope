...
list_selection_does_not_shift_item_text_test() ->
    ...
    Screen = iso_screen:from_ansi(iso_render:render(Tree, Bounds), 12, 3),
...
text_wrap_uses_render_bounds_width_test() ->
    ...
    Screen = iso_screen:from_ansi(iso_render:render(Tree, Bounds), 6, 3),
...
scroll_offset_counts_wrapped_text_lines_test() ->
    ...
    Screen = iso_screen:from_ansi(iso_render:render(Tree, Bounds), 4, 2),
...
screen_diff_identical_screens_is_empty_test() ->
    Screen = iso_screen:new(8, 2),
    ?assertEqual([], iso_screen:diff(Screen, Screen)).

screen_diff_coalesces_adjacent_cell_updates_test() ->
    OldScreen = iso_screen:new(8, 1),
    NewScreen = iso_screen:put_string(OldScreen, 0, 0, <<"abc">>, #{}),
    Diff = iolist_to_binary(iso_screen:diff(OldScreen, NewScreen)),
